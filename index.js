...
class EklaseWrapper {
	...
	async initialize(keepAlive = false) {
		...
		let aspxauth = data.headers["set-cookie"][0] // The cookie is given in the 302 request in the set-cookie header

		this.headers = {
			Cookie: `${cookie.name}=${/\.ASPXAUTH.{1,99}=([^;]{1,9999})/.exec(aspxauth)[1]}`
		}

		if (keepAlive) setInterval(() => { // Necessary for code that wants to be ran for more than a few mins
